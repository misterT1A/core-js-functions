...
function getArgumentsCount(funcs) {
  return funcs.map((elem) => elem.length);
}
...
function getPolynom(a, b, c) {
  ...
  if (arguments.length === 2) return (x) => a * x + b;
  if (arguments.length === 3) return (x) => a * x ** 2 + b * x + c;
  return null;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const string = args.map((elem) => JSON.stringify(elem));
    const message = `${func.name}(${string})`;
